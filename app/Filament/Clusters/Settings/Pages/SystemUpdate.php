        ...
        return [
            ActionGroup::make([
                Action::make('saveGitHubToken')
                    ...
                    ->form([
                        TextInput::make('token')
                            ->label('Kode akses pembaruan')
                            ->password()
                            ->revealable()
                            ->required()
                            ->minLength(20)
                            ->helperText('Tempel token dari pengelola teknis. Token disimpan aman dan hanya dipakai untuk pembaruan.'),
                    ])
                    ->modalHeading('Ganti akses pembaruan')
                ...
                Action::make('forgetGitHubToken')
                    ...
                    ->modalHeading('Hapus akses pembaruan?')
                    ->modalDescription('Sistem tidak dapat mengecek pembaruan sampai kode akses baru disimpan.')
                    ->modalSubmitActionLabel('Hapus Akses')
                    ->action(fn (): null => $this->forgetGitHubToken()),
            ])
                ->label('Akses Pembaruan')
                ->icon(Heroicon::OutlinedKey)
                ->button()
                ->color('gray')
            ...
            Action::make('saveGitHubToken')
                ...
                ->modalHeading('Atur akses pembaruan')
                ->modalSubmitActionLabel('Simpan Akses')
                ->action(fn (array $data): null => $this->saveGitHubToken($data)),
